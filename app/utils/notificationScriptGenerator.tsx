...
export function generateEmbedScript(notifications: Notification[]): string {
  ...
  const notificationsJSON = JSON.stringify(notifications.map(n => ({
    title: n.title,
    body: n.message, // Changed from n.body to n.message to match the interface
    icon: n.image || null, // Changed from n.icon to n.image to match the interface
    url: n.url || null,
    duration: 5000, // Default duration as this property doesn't exist in the interface
    position: 'top' // Default position as this property doesn't exist in the interface
  })));
