  ...
  const fetchInvoices = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const invoices = Array.isArray(data.transactions)
          ? data.transactions.map((inv: Invoice, idx: number) => ({
              ...inv,
              // Generate invoice number if missing or N/A
              number: inv.number && inv.number !== 'N/A' ? inv.number : `INV-${(idx + 1).toString().padStart(4, '0')}`,
              // Format created and dueDate to readable format
              created: inv.created ? new Date(inv.created).toLocaleDateString('en-GB', { year: 'numeric', month: 'short', day: '2-digit' }) : '',
              dueDate: inv.dueDate ? new Date(inv.dueDate).toLocaleDateString('en-GB', { year: 'numeric', month: 'short', day: '2-digit' }) : (inv.created ? new Date(inv.created).toLocaleDateString('en-GB', { year: 'numeric', month: 'short', day: '2-digit' }) : '')
            }))
          : [];
